...
export function Header() {
  ...
  const links = [
    { id: "process", label: tr.nav.how },
    { id: "who", label: tr.nav.who },
    { id: "infra", label: tr.nav.infra },
  ]
    ...
    <header className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-4 px-4">
      ...
      >
        <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="flex items-center gap-2 text-sm font-bold tracking-tight text-black hover:text-[#00FF88] transition-colors shrink-0">
          <span className="text-[#00FF88]">{">"}</span> {PRODUCT.name}
        </button>
        ...
        <button onClick={() => scrollTo("waitlist")} className="hidden lg:inline-flex items-center gap-1.5 bg-[#00FF88] text-black font-medium px-4 py-1.5 text-xs hover:bg-black hover:text-white transition-colors shrink-0" style={{ borderRadius: "999px" }}>
          {tr.nav.waitlist} →
        </button>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}
            ...
            <button onClick={() => scrollTo("waitlist")} className="w-full text-center px-4 py-3 mt-1 text-sm font-medium bg-[#00FF88] text-black hover:bg-black hover:text-white transition-colors" style={{ borderRadius: "16px" }}>
              {tr.nav.waitlist} →
            </button>
          </motion.div>
        )}

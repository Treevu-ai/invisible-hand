    ...
    <section className="relative py-20 px-6 text-center bg-white">
      <div className="max-w-[600px] mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }} className="flex flex-col gap-6">
          ...
          <h2 className="text-[clamp(2rem,4vw,3rem)] font-bold tracking-[-0.02em] text-black [text-shadow:0_0_18px_rgba(0,255,136,0.18)]">{tr.headline}<br />{tr.headline2}</h2>
          <div className="flex flex-col sm:flex-row gap-3 justify-center mt-6">
            ...
            <a href={`https://t.me/${CONTACT.telegram.replace("+", "")}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 border border-gray-300 text-gray-700 px-10 py-4 text-sm hover:bg-gray-50 hover:text-black transition-colors font-medium" style={{ borderRadius: "999px" }}>{tr.secondary}</a>
          </div>
        </motion.div>
      </div>
    </section>

    ...
    <section id="process" className="relative py-24 px-6 bg-[#F8F8F8]">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 relative">
          <svg className="absolute top-[60px] left-[12.5%] w-[75%] h-0 overflow-visible hidden lg:block">
            <path d="M0,0 Q130,-30 260,0 Q390,30 520,0 Q650,-30 780,0" fill="none" stroke="#00FF88" strokeWidth="0.5" opacity="0.2" strokeDasharray="4 6"/>
          </svg>
          {tr.steps.map((step, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.12, ease: [0.16, 1, 0.3, 1] }} className="flex flex-col items-center text-center gap-4 p-6 relative">
              <div className="flex items-center justify-center w-16 h-16 border-2 border-[#00FF88]/20 bg-[#00FF88]/5" style={{ borderRadius: "50%" }}>
                <span className="text-[#00FF88] text-xl font-bold">{step.n}</span>
              </div>

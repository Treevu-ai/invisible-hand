...
export function ProblemSolution() {
  ...
  return (
    <section id="problem" className="relative py-24 px-6 border-t border-gray-100 dark:border-white/5 bg-[#F8F8F8] dark:bg-[#0A0A0A]">
      <div className="max-w-[1200px] mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }} className="flex flex-col gap-3 mb-16">
          ...
          <h2 className="text-[clamp(2rem,4vw,3rem)] font-bold leading-[1.1] max-w-[700px] tracking-[-0.02em] text-black dark:text-white [text-shadow:0_0_18px_rgba(0,255,136,0.18)]">{tr.headline}<br />{tr.headline2}</h2>
          <p className="text-gray-600 dark:text-gray-300 text-lg leading-relaxed max-w-[600px] mt-2">{tr.subtext}</p>
        </motion.div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {tr.cards.map((card, i) => {
            const Icon = icons[i]
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1, ease: [0.16, 1, 0.3, 1] }} className="group border border-gray-100 dark:border-white/5 bg-white dark:bg-white/[0.02] p-8 hover:border-[#00FF88]/30 hover:shadow-sm transition-all duration-300" style={{ borderRadius: "40px" }}>
                <Icon /><div className="mt-5" />
                <h3 className="font-bold text-black dark:text-white text-lg mb-2 tracking-tight">{card.title}</h3>
